...
- `run`, `submit`, `status`, `fetch` and `cleanup` subcommands over a durable
  `_state.json`, written atomically before it is acted on. An interrupted run no
  longer loses a submitted (and paid for) batch job.
- Uploaded originals are deleted on every exit path, not only on success, and
  carry a server-side expiry as a backstop. `cleanup` retries failed deletions
  and refuses to delete files a running job still needs.
